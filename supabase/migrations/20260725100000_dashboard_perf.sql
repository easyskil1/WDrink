-- =============================================================================
-- Dashboard teljesítmény (Feladatlista 2 / B fázis)
--   B2: a beszállító- és vevő-rangsort SQL-ben számoljuk (top 10), hogy a
--       statisztika oldal NE húzza le a teljes movement_log-ot JS-aggregációhoz.
--   B3: az idősor alszűrői dátumkorlátot kapnak (utolsó 30 nap), hogy ne
--       csoportosítsák a teljes előzményt minden betöltéskor.
-- A visszaadott kulcsok bővülnek: beszallito_rangsor, vevo_rangsor
--   (mindkettő [{label, value}] top 10, HBars-kompatibilis).
-- =============================================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    -- B2: Beszállítók rangsora bevételezési (beszerzési) érték szerint, top 10
    'beszallito_rangsor', coalesce((
      select jsonb_agg(x)
      from (
        select jsonb_build_object('label', s.nev, 'value',
          round(sum(m.mennyiseg * coalesce(pu.beszerzesi_ar, 0)
            / nullif(pu.mennyiseg_alapegysegben, 0)))) as x
        from movement_log m
        join delivery_notes dn on dn.id = m.delivery_note_id
        join suppliers s on s.id = dn.supplier_id
        join stock_items si on si.id = m.stock_item_id
        join product_units pu on pu.id = si.product_unit_id
        where m.tipus = 'bevetelezes'
        group by s.nev
        order by sum(m.mennyiseg * coalesce(pu.beszerzesi_ar, 0)
          / nullif(pu.mennyiseg_alapegysegben, 0)) desc nulls last
        limit 10
      ) t
    ), '[]'::jsonb),

    -- B2: Vásárlók rangsora kiszállítási (nettó eladási) érték szerint, top 10
    'vevo_rangsor', coalesce((
      select jsonb_agg(x)
      from (
        select jsonb_build_object('label', dn.vevo_nev, 'value',
          round(sum(m.mennyiseg * coalesce(pu.netto_ar, 0)
            / nullif(pu.mennyiseg_alapegysegben, 0)))) as x
        from movement_log m
        join delivery_notes dn on dn.id = m.delivery_note_id
        join stock_items si on si.id = m.stock_item_id
        join product_units pu on pu.id = si.product_unit_id
        where m.tipus = 'kiadas' and dn.vevo_nev is not null
        group by dn.vevo_nev
        order by sum(m.mennyiseg * coalesce(pu.netto_ar, 0)
          / nullif(pu.mennyiseg_alapegysegben, 0)) desc nulls last
        limit 10
      ) t
    ), '[]'::jsonb),
    ...
    -- B3: az alszűrők is csak az utolsó 30 napot csoportosítják.
    'idosor', coalesce((
      ...
      from (
        ...
        left join (
          ...
          from movement_log
          where tipus='bevetelezes' and created_at >= current_date - interval '29 days'
          group by 1
        ) b on b.nap = d.nap::date
        left join (
          select date_trunc('day', created_at)::date nap, sum(mennyiseg) db
          from movement_log
          where tipus='kiadas' and created_at >= current_date - interval '29 days'
          group by 1
